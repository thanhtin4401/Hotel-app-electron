...
const ListPromotionPage = () => {
  const isDeleteSuccess = useSelector(
    (state) => state.manager?.user?.isDeleteSuccess
  );
  const isUpdateSuccess = useSelector(
    (state) => state.manager?.user?.isUpdateSuccess
  );

  const dispatch = useDispatch();
  const isRegisterAccountSuccess = useSelector(
    (state) => state.auth.isRegisterAccountSuccess
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record, index) => (
        <div>{moment(record.NgayBatDau).format("dd / mm / yyyy")}</div>
      ),
    },
    {
  ...
  return (
    <>
      <div className="w-full text-left p-2 bg-white rounded-[2rem] mb-3">
        <h1 className="text-[#1c305e] ml-4 text-[1rem] font-[700]">
